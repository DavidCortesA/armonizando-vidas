  ...
  const brandName =
    theme.key === "main"
      ...
      : theme.key === "foodBank"
        ...
        : t("brand.orchestra");
  const brandSubtitle =
    theme.key === "main" ? t("brand.mainSubtitle") : t("brand.parent");
  const homePath =
    theme.key === "foodBank"
      ? PATHS.FOOD_BANK
      : theme.key === "orchestra"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-18 py-3">
              ...
              {theme.key === "main" && (
                <div className="hidden sm:block">
                  <div
                    ...
                    style={{
                      color: isDark || !scrolled ? "#F8FAFC" : theme.navText,
                    }}
                  >
                    {brandName}
                  </div>
                  <div
                    ...
                    style={{
                      color: isDark || !scrolled ? "#CBD5E1" : theme.navText,
                    }}
                  >
                    {brandSubtitle}
                  </div>
                          ...
                          {item.children.map((child) => (
                            ...
                            >
                              <span
                                ...
                                style={{
                                  background: child.path.includes("banco")
                                    ? "#F59E0B"
                                    : "#6D28D9",
                                }}
                              />
            ...
            <div className="flex items-center gap-3">
              <div
                className="hidden sm:flex items-center rounded-full border border-current/10 overflow-hidden"
              ...
              >
                {(["es", "en"] as const).map((lng) => (
                  <button
                    ...
                    style={{
                      background:
                        i18n.language === lng ? theme.navAccent : "transparent",
                      color:
                        i18n.language === lng
                          ? theme.key === "orchestra"
                            ? "#0D0D1A"
              ...
              {theme.key === "orchestra" && (
                <Link
                  to={PATHS.DONATE}
                  className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 hover:scale-105"
                  style={{ background: "#FCD34D", color: "#0D0D1A" }}
                >
                  <Heart size={14} />
                  {t("nav.support")}
                </Link>
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                ...
                borderLeft: `1px solid ${theme.navBorder}`,
              }}
            >
              <div
              ...
              >
                <span
                  className="font-bold"
                  style={{ color: isDark ? "#F8FAFC" : theme.navText }}
                >
                  {t("nav.menu")}
                </span>
                <button
                  onClick={() => setMobileOpen(false)}
                  style={{ color: isDark ? "#94A3B8" : theme.navText }}
                >
                  <X size={20} />
                </button>
              </div>
                        ...
                        {item.children.map((child) => (
                          ...
                          >
                            <span
                              ...
                              style={{ background: theme.navAccent }}
                            />
                            {t(child.label)}
                          </Link>
                        ))}
              ...
              >
                <Link
                  ...
                  style={{
                    background: theme.navAccent,
                    color: theme.key === "orchestra" ? "#0D0D1A" : "#0A1F44",
                  }}
                >
                  <Heart size={16} />
                  {t("nav.makeDonation")}
                ...
                <div
                  className="mt-3 grid grid-cols-2 rounded-xl border overflow-hidden"
                  style={{ borderColor: theme.navBorder }}
                >
                  {(["es", "en"] as const).map((lng) => (
                    <button
